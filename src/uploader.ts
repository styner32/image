...
export default class Uploader {
   ...
   * Select a file
   */
  public selectFile({ onPreview }: UploadOptions): void {
    const preparePreview = function (file: File): void {
      ...
      reader.onload = (e) => {
        ...
        const img = new Image();
        img.onload = () => {
          onPreview(imgSrc, img.naturalWidth, img.naturalHeight);
        };
        img.src = imgSrc;
      };
    };
  ...
  public uploadFile(options?: { width?: number; height?: number }) {
    ...
    if (!this.config.uploader) {
      console.log("no uploader config");
      return;
    }
    ...
    return this.config.uploader
      .uploadByFile?.(this.selectedFile, options)
      .then((response) => {
