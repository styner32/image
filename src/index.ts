...
export default class ImageTool implements BlockTool {
  ...
  constructor({
    ...
    this.uploader = new Uploader({
      config: this.config,
      onUpload: (response: UploadResponseFormat) => {
    ...
    this.ui = new Ui({
      ...
      onSelectFile: () => {
        this.uploader.selectFile({
          onPreview: (src: string, width?: number, height?: number) => {
            this.ui.showPreloader(src, width, height);
          },
        });
      },
      onUploadFile: () => {
        this.uploader.uploadFile(this.ui.getSize());
        this.ui.showSpinner();
      },
      onResize: (type: "up" | "down") => {
        this.ui.resizeImage(type);
      },
      readOnly,
    });
